...
public class BluetoothManager : NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    //    public var connectedCompletion: ((CBPeripheral) -> Void)?
    public var responseCompletion: ((Data?) -> Void)?
    public var discoveredCharacteristicCompletion: ((CBService) -> Void)?
    public var discoveredServicesCompletion: ((CBPeripheral) -> Void)?
    ...
    func startScanPeripheral() {
        _manager?.scanForPeripherals(withServices: [CBUUID.init(string: "0000fff0-0000-1000-8000-00805f9b34fb")], options: [CBCentralManagerScanOptionAllowDuplicatesKey:true])
    }
...
func disconnectPeripheral() {
    ...
    func writeValue(data: Data, forCharacteristic characteristic: CBCharacteristic, type: CBCharacteristicWriteType) {
        ...
        print("发送:\(String(format: "%@", data as CVarArg))")
    ...
    public func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        peripheral.discoverServices([CBUUID.init(string: "0000fff0-0000-1000-8000-00805f9b34fb")])
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        connectedPeripheral = peripheral
        if error != nil {
            print("Bluetooth Manager --> Discover Services Error, error:", error?.localizedDescription as Any)
            return ;
        }
        ...
        if discoveredServicesCompletion != nil {
            discoveredServicesCompletion!(peripheral)
        } else {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        if discoveredCharacteristicCompletion != nil {
            discoveredCharacteristicCompletion!(service)
        } else {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?){
        if error == nil {
            self.delegate?.didUpdateNotificationStateFor?(characteristic)
        } else {
            return
        }
    }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        responseCompletion?(characteristic.value)
